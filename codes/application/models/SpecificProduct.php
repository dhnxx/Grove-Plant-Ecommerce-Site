...
class SpecificProduct extends CI_Model
{
    ...
    public function get_product($product_id)
    {
        ...
        $query = "SELECT products.*, ROUND(AVG(product_reviews.rating)) 
        AS avg_rating, count(product_reviews.id) AS review_count 
        FROM products LEFT JOIN product_reviews ON products.id = product_reviews.product_id 
    ...
    public function get_similar_products($category_id, $product_id)
    {
        ...
        WHERE category_id = ? AND products.id != ? 
    ...
    public function get_reviews($product_id)
    {
        ...
        $query = "SELECT product_reviews.*, CONCAT(users.first_name , ' ', users.last_name) as name FROM product_reviews  
        LEFT JOIN users ON product_reviews.user_id = users.id 
        WHERE product_id = ? ORDER BY product_reviews.created_at DESC";
        $value = array($product_id);
        return $this->db->query($query, $value)->result_array();
    }
